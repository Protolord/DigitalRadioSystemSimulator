...
testdata_max = [
    (6,
     numpy.array([6, 2 + 5j, 5j, -4 + 4j, -5, -4 - 3j, -5j, 5 - 2j])),
    (5 + 4j,
     numpy.array([6, 5 + 4j, 6j, -3 + 5j, -6, -4 - 4j, -5j, 6 - 2j])),
    (6j,
     numpy.array([5, 5 + 2j, 6j, -3 + 4j, -5, -4 - 4j, -5j, 5 - 2j])),
    (-4 + 5j,
     numpy.array([6, 3 + 5j, 6j, -4 + 5j, -6, -4 - 4j, -6j, 2 - 6j])),
    (-6,
     numpy.array([5, 5 + 2j, 5j, -5 + 3j, -6, -3 - 4j, -5j, 2 - 5j])),
    (-4 - 5j,
     numpy.array([6, 4 + 3j, 6j, -6 + 2j, -6, -4 - 5j, -6j, 6 - 2j])),
    (-6j,
     numpy.array([5, 2 + 5j, 5j, -4 + 3j, -5, -3 - 4j, -6j, 2 - 5j])),
    (4 - 5j,
     numpy.array([6, 4 + 4j, 6j, -2 + 6j, -6, -3 - 5j,  6j, 4 - 5j]))
]
...
@pytest.mark.parametrize(
    'expected, data, range_values, data_type', testdata_check_input_validity
)
def test_check_input_validity(expected, data, range_values, data_type):
    assert (expected
            == utils.check_input_validity(data, range_values, data_type))


testdata_find_index = [
    # array([0., 0.111, 0.222, 0.333, 0.444, 0.556, 0.667, 0.777, 0.889, 1.])
