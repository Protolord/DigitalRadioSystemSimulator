...
def test_fulltime_multiple_add(channel):
    ...
    assert (signal1 + signal2 + signal3
            == channel.signal_get(0, SIM_DURATION)).all()
...
def test_parttime_single_add(channel):
    ...
    assert (numpy.zeros(index_half)
            == channel.signal_get(0, SIM_DURATION/2 - TIME_STEP)).all()
    assert (signal[:index_half]
            == channel.signal_get(SIM_DURATION/2, SIM_DURATION)).all()
...
def test_parttime_multiple_add(channel):
    ...
    actual_quarter1 = channel.signal_get(
        0, SIM_DURATION/4 - TIME_STEP
    )
    actual_quarter2 = channel.signal_get(
        SIM_DURATION/4, 2*SIM_DURATION/4 - TIME_STEP
    )
    actual_quarter3 = channel.signal_get(
        2*SIM_DURATION/4, 3*SIM_DURATION/4 - TIME_STEP
    )
    actual_quarter4 = channel.signal_get(
        3*SIM_DURATION/4, SIM_DURATION
    )
